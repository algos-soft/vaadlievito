...
As you notice, the value of the `@Route' annotation for the reviews view is empty, which means it is the *default* route target for this application.

[NOTE]
`MainLayout.class` is declared as the parent layout here.
In order for a class to be used as a parent layout, it must implement the `RouterLayout` interface.

In annotation based router API, the servlet will be registered automatically by mapping the `@Route` annotation from each navigation target, so there is no need to declare it manually.
